<x-app-layout>
        ...
        <div class="flex items-center justify-between">
            <div>
                <h2 class="font-semibold text-2xl text-slate-900 dark:text-slate-100 leading-tight">
                    {{ __('staff.edit_qualification') }}
                </h2>
                <p class="text-sm text-slate-600 dark:text-slate-300 mt-1">{{ $carer->name }} · {{ $qualification->name }}</p>
            </div>
    ...
    <div class="py-8">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('manager.carers.qualifications.update', [$carer, $qualification]) }}"
                  enctype="multipart/form-data"
                  class="rounded-2xl border border-slate-200 bg-white shadow-sm p-6 space-y-4">
                @csrf
                @method('PUT')

                @include('manager.qualifications._form', ['qualification' => $qualification])

                @if($qualification->document_path)
                    <div class="text-sm">
                        <span class="text-slate-500">{{ __('staff.current_document') }}:</span>
                        <a href="{{ Storage::disk('public')->url($qualification->document_path) }}"
                           target="_blank" class="text-blue-600 hover:underline ml-2">
